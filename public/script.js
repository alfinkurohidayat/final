...
// ==============================
// 🌍 GLOBAL CONFIG (TAMBAHAN BARU - TIDAK MENGHAPUS APAPUN)
// ==============================
const BASE_URL = "https://final-9pgj.onrender.com";

function buildUrl(path) {
  if (!path) return "";
  if (path.startsWith("http")) return path; // untuk cloudinary
  return BASE_URL + path; // untuk local upload
}
  ...
  try {
    ...
    media.forEach((item) => {
      ...
      tr.innerHTML = `
        ...
        <td>${escapeHtml(item.kelas || "-")}</td>
        <td>${escapeHtml(item.submateri || "-")}</td>
        <td>${item.url ? `<a href="${buildUrl(item.url)}" target="_blank">Lihat</a>` : "-"}</td>
        <td>
          <button class="editBtn" data-id="${item._id}">Edit</button>
          <button class="deleteBtn" data-id="${item._id}">Hapus</button>
        </td>
...
// ==============================
// POPUP OVERLAY (TETAP SAMA + STABIL)
// ==============================
function openOverlay(type, url) {
  const modal = document.getElementById("mediaModal");
  const body = document.getElementById("modalBody");
  ...
  if (type === "image") {
    body.innerHTML = `<img src="${buildUrl(url)}" style="width:100%">`;
  }

  if (type === "video") {
    body.innerHTML = `
      <video controls style="width:100%">
        <source src="${buildUrl(url)}">
      </video>
    `;
  }
