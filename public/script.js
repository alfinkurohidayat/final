...
//       const urlDisplay = item.url ? buildUrl(item.url) : "-";
//       tr.innerHTML = `
...
//           openOverlay(item.overlayType, buildUrl(item.overlayUrl));
//         });
...
//   return "https://final-9pgj.onrender.com" + path; // local
// }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // cek login (hanya jika loginSection/adminSection ada)
  if (loginSection || adminSection) {
    try {
      const res = await fetch("https://final-9pgj.onrender.com/check-login", {
        credentials: "include",
      ...
      const data = await res.json();
      if (data.loggedIn && loginSection && adminSection) {
        loginSection.style.display = "none";
        adminSection.style.display = "block";
        if (logoutBtn) logoutBtn.style.display = "inline-block";
        if (typeof loadMediaList === "function") await loadMediaList();
      }
    } catch (err) {
      console.error("Gagal cek status login:", err);
    }
  ...
  if (loginBtn) {
    loginBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const data = await res.json();
        if (res.ok && data.success && loginSection && adminSection) {
          if (msg) {
            msg.textContent = "Login berhasil!";
            msg.style.color = "green";
          }
  ...
  try {
    ...
    tbody.querySelectorAll(".editBtn").forEach((b) => {
      b.addEventListener("click", (e) => {
        ...
        if (editIdEl) editIdEl.value = item._id;
        if (titleEl) titleEl.value = item.title || "";
        if (typeEl) typeEl.value = item.type || "";
        if (classLevelEl) classLevelEl.value = item.kelas || "";
        if (categoryEl) categoryEl.value = item.submateri || "";
        const addBtnGlobal = document.getElementById("addBtn");
        if (addBtnGlobal) addBtnGlobal.textContent = "Update Media";
    ...
    console.error("Gagal memuat daftar media:", err);
    if (tbody)
      tbody.innerHTML = `<tr><td colspan="6">Error load media</td></tr>`;
  }
}

// ==============================
// 🧭 Fungsi Navigasi & Tampilkan Materi (Index.html)
// ==============================
document.addEventListener("DOMContentLoaded", () => {
  const DM = document.getElementById("DM");
  const daftarMateri = document.getElementById("daftar-materi");
  ...
  if (!DM || !daftarMateri || !submenu || !submenuTitle || !submenuContent)
    return; // skip jika halaman admin
  ...
  async function tampilkanMateri(kelas, submateri) {
    ...
    try {
      const res = await fetch(
        `https://final-9pgj.onrender.com/api/media/${encodeURIComponent(
          kelas,
        )}/${encodeURIComponent(submateri)}`,
      );
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();

      if (daftarMateri) daftarMateri.innerHTML = "";
      if (!data || data.length === 0) {
        if (daftarMateri)
          daftarMateri.innerHTML = "<p>Tidak ada materi tersedia.</p>";
        return;
      ...
      data.forEach((item) => {
        ...
        img.addEventListener("click", () => {
          if (!item.overlayType || !item.overlayUrl) return;
          openOverlay(item.overlayType, buildUrl(item.overlayUrl));
        });
  ...
  async function tampilkanMediaApps() {
    ...
    try {
      ...
      data.forEach((item) => {
        ...
        let mediaPreview = "";
        if (item.type === "video") {
          mediaPreview = `<video width="100%" height="200" controls src="${buildUrl(
            item.url,
          )}"></video>`;
        } else if (item.type === "audio") {
          mediaPreview = `<audio controls src="${buildUrl(item.url)}"></audio>`;
        }
...
function buildUrl(path) {
  ...
  return "https://final-9pgj.onrender.com" + path;
}
